...
gillespied_mtDNA = function(N, T.sim, dt.sim, ...){
  ...
  repeat{
    ...
    while( tt >= target ){
      ...
      if( i>n ){
        return( list("population"=xmat, "hazards"=hvec) )
      }
    }
...
gillr_time1 = system.time({
  lapply(NN, gillespied_mtDNA, T.sim, dt.sim)
})
...
avg_hazards = function(raw_sim){
  Nsim = length(raw_sim)
  hazard_avgs = double(Nsim)
  for(i in 1:Nsim){
    hazard_avgs[i] = mean(raw_sim[[i]]$hazards)
  }
  return(hazard_avgs)
}

mode_kde = function(vec){
  density(vec)$x[which.max(density(vec)$y)]
}


havg = avg_hazards(Gillespie_sims)

par(mfrow=c(1,1))
plot(density(havg), main=expression(h[0]~" Density"), ylab="", xlab=expression(h[0]), lwd=2)
abline(v=mean(havg), col="blue", lwd=2)
abline(v=mode_kde(havg), col="red", lwd=2)
