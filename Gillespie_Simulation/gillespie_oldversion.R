...
N$h = function(x, error, K_c=8.99e-9,
  ...
  if(error>=0){
    with(as.list(c(x,th.1)), {
      return( c(th.1[1]*x[1], th.1[2]*x[2], th.1[3]*x[1], th.1[4]*x[2], th.1[5]*x[1] ))
    })
  } else {
    if(th.1[1]<0) th.1[1] = K_c
    if(th.1[2]<0) th.1[2] = K_c
    with(as.list(c(x,th.1)), {
      return( c(th.1[1]*x[1], th.1[2]*x[2], th.1[3]*x[1], th.1[4]*x[2], th.1[5]*x[1] ))
    })
  }
...
GillespieSims_f = lapply(NN, gillespied, T.sim, dt.sim)
